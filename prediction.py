...
def main(args):
    ...
    frame_id = 1
    future_trajectories = defaultdict(list)  # Store future trajectory data for each track ID
    image_history = []  # Store the images for the past frames
    ...
    while True:
        ...
        if len(image_history) == 6:
            # Use the first image in history as the base image
            display_image = image_history[0].copy()

            # Plot future bounding box centers on the display image
            for track_id, future_points in future_trajectories.items():
            ...
            # Display the canvas with the base image and future trajectories
            canvas = np.ones((canvas_height, canvas_width, 3), dtype=np.uint8) * 255
            canvas[0:video_display_height, 0:video_display_width] = display_image
        ...
        # Store the current frame into history
        frame_resized_for_history = cv2.resize(frame, (video_display_width, video_display_height))
        image_history.append(frame_resized_for_history)
        ...
        future_trajectories.clear()  # Clear future trajectory data for this frame
        ...
        for det in dets:
            ...
            if cv2.pointPolygonTest(roi_points, center, False) >= 0:
                ...
                # Store the future bounding box center
                future_trajectories[det.track_id].append(center)
...
parser.add_argument('--vmax', type=float, default=5, help='vmax')
